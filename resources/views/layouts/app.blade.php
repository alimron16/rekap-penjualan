            ...
            <nav class="flex-1 overflow-y-auto px-3 py-3 space-y-1 text-xs">
                ...
                <div class="pt-2">
                    ...
                    <div class="space-y-0.5">
                        ...
                        <a href="{{ route('master.outlets.index') }}" class="group flex items-center gap-2.5 px-3 py-1.5 rounded-lg transition {{ request()->routeIs('master.outlets.*') ? 'bg-emerald-600 text-white font-bold shadow-xs' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                            <svg class="w-4 h-4 text-white flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                ...
                @if(auth()->check() && (auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('purchase')))
                <!-- 2. Pembelian Group -->
                <div class="pt-2">
                    <div class="px-3 pb-1 text-[10px] font-bold text-white/70 uppercase tracking-wider flex items-center justify-between">
                        <span>Pembelian</span>
                ...
                <div class="pt-2">
                    ...
                    <div class="space-y-0.5">
                        ...
                        <a href="{{ route('pos.shift') }}" class="group flex items-center gap-2.5 px-3 py-1.5 rounded-lg transition {{ request()->routeIs('pos.shift') ? 'bg-emerald-600 text-white font-bold shadow-xs' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                            <svg class="w-4 h-4 text-white flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <span class="flex-1">Rekap Shift & Setor</span>
                            <span class="text-[9px] bg-emerald-500/30 text-emerald-200 px-1.5 py-0.5 rounded font-mono font-bold">Shift</span>
                        </a>
                        ...
                        @if(auth()->check() && (auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('accounting')))
